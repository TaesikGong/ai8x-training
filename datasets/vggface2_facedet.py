...
#         """
#         mtcnn = MTCNN()
#         pattern = re.compile(r'n00000\d{1}/.*\.jpg')
#         img_paths = [path for path in glob.glob(os.path.join(self.d_path + '/**/*.jpg'), recursive=True) if
#                      pattern.search(path)]
...
class VGGFace2_FaceDetectionDataset(Dataset):
    ...
    def __extract_gt(self):
        ...
        pattern = re.compile(r'n0000\d{2}/.*\.jpg')
        img_paths = [path for path in glob.glob(os.path.join(self.d_path + '/**/*.jpg'), recursive=True) if
                     pattern.search(path)]
        nf_number = 0
        pickle_dict = {key: [] for key in ["gt", "img_list"]}
...
def VGGFace2_Facedet_get_datasets(data, load_train=True, load_test=True, input_size=180, target_size=64, target_channel=3):
    ...
    if load_train:
        ...
        train_dataset = VGGFace2_FaceDetectionDataset(root_dir=data_dir, d_type='train',
                                                      transform=train_transform, img_size=(input_size, input_size))
    ...
    if load_test:
        ...
        test_dataset = VGGFace2_FaceDetectionDataset(root_dir=data_dir, d_type='test',
                                                     transform=test_transform, img_size=(input_size, input_size))
...
for size in [64]:
    for channel in range(65):
        dic = {}
        dic['name'] = f'VGGFace2_FaceDetection_{channel}x{size}x{size}'
        dic['input'] = (channel, size, size)
        dic['output'] = ([1])
        dic['loader'] = partial(VGGFace2_Facedet_get_datasets, load_train=True, load_test=True, input_size=initial_image_size,
                                target_size=size, target_channel=channel)
        dic['collate'] = VGGFace2_FaceDetectionDataset.collate_fn
        datasets.append(dic)
